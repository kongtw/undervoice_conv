...
import conv_spk.evaluation.nets.nets_factory as nets_factory_ev_spk
...
tf.app.flags.DEFINE_string(
    'enrollment_dir_spk', './conv_ampli/results/Model',
    'Directory where checkpoints and event logs are written to.')
...
tf.app.flags.DEFINE_string(
    'evaluation_dir_spk', './conv_ampli/results/ROC',
    'Directory where checkpoints and event logs are written to.')
...
def determine_spk(sc_file):
    kf_flag = []
    kf_score = []
    ubm_score = []
    with open(sc_file, 'r') as sc_fp:
        ct = 0
        sc_lst = []
        while True:
            line = sc_fp.readline()
            if line == '':
                ...
                ct += 1
                sc_str = line.split(' ')[4]
                if sc_str.find('nan') != -1:
                    sc_lst.extend([-100])
                else:
                    sc = float(line.split(' ')[4])
                    sc_lst.extend([sc])
                if ct % 2 == 0:
                    kf_score.extend([sc_lst[0]])
                    ubm_score.extend([sc_lst[1]])
                    if sc_lst[1] > sc_lst[0]:
                        kf_flag.extend([0])
                    else:
                        kf_flag.extend([1])
                    sc_lst = []
                    ...
                    pass
    return [kf_flag, kf_score, ubm_score]


def spk_sp(chunk_name_list, cursor, cfg_list):
    [dest_norm_cfg, dest_norm_spro_cfg, dest_en_cfg, dest_iv_cfg, dest_test_cfg, dest_norm_ndx] = cfg_list
    ...
    ndx_dir = '/home/dell/runtimedata/spk_test_data/ndx_rt/'
    ndx_name = ndx_dir + str(os.getpid()) + '_ivExtractor.ndx'
    test_ndx_name = ndx_dir + str(os.getpid()) + '_ivTest_plda_target-seg.ndx'
    model_ndx_name = ndx_dir + str(os.getpid()) + '_trainModel.ndx'
    spk_num = 2
    ...
    if kf_num[0][0] == 0:
        ...
        with open(model_ndx_name, 'w') as model_ndx_fp:
            model_ndx_fp.writelines('spk01 %s\n' % kf_id)
            model_ndx_fp.writelines('spk02 %s\n' % 'ubm')
        delete_list = []
        with open(ndx_name, 'w') as ndx_fp:
            with open(test_ndx_name, 'w') as test_ndx_fp:
                for wav_name in chunk_name_list:
                    print wav_name
                    wav_index = wav_name[0].split('/')[-1].split('.')[0] + '_' + str(os.getpid())
                    ndx_fp.writelines(wav_index + ' ' + wav_index + '\n')
                    test_ndx_fp.writelines(wav_index)
                    for i in xrange(1, spk_num):
                        test_ndx_fp.writelines(' spk%02d' % i)
                    test_ndx_fp.writelines(' spk%02d' % spk_num + '\n')
                    cm_str = './spk_sep/bin/sfbcep -f 8000 -m -k 0.97 -p19 -n 24 -r 22 -e -D -A -F PCM16  %s ' \
                             '%s/%s.tmp.prm' % (wav_name[0], prm_dir, wav_index)
                    os.system(cm_str)
                    delete_list.extend(['%s/%s.tmp.prm' % (prm_dir, wav_index)])

                    cm_str = './spk_sep/bin/NormFeat --config %s --inputFeatureFilename %s ' \
                    ...
                    if os.path.exists(lbl_dir + wav_index + '.lbl'):
                        ...
                        cm_str = './spk_sep/bin/EnergyDetector  --config %s --inputFeatureFilename %s ' \
                                 '--featureFilesPath  %s  --labelFilesPath  %s' % (
                                     dest_en_cfg, wav_index, prm_dir, lbl_dir)
                        os.system(cm_str)
                    else:
                        cm_str = './spk_sep/bin/EnergyDetector  --config %s --inputFeatureFilename %s ' \
                                 '--featureFilesPath  %s  --labelFilesPath  %s' % (
        ...
        cm_str = './spk_sep/bin/IvExtractor --config %s --saveVectorFilesPath %s --featureFilesPath %s --labelFilesPath %s ' \
        ...
        res_file_name = './spk_sc/' + str(os.getpid()) + '_res.txt'
        cm_str = './spk_sep/bin/IvTest --config %s --outputFilename %s --ndxFilename %s --targetIdList %s --loadVectorFilesPath %s ' \
                 '--testVectorFilesPath %s --backgroundNdxFilename %s' \
                 % (dest_test_cfg, res_file_name, test_ndx_name, model_ndx_name, iv_raw, iv_raw,
                    dest_norm_ndx)
        os.system(cm_str)
        iv_res = determine_spk(res_file_name)
...
def prepare_spksp():
    ...
    cfg_dict['ivExtractor'] = 'ivExtractor_fast'
    cfg_dict['IvTest'] = 'ivTest_WCCN_Cosine'
    ...
    return [dest_norm_cfg, dest_norm_spro_cfg, dest_en_cfg, dest_iv_cfg, dest_test_cfg, dest_norm_ndx]
...
class predict_ampli_conv:
    ...
    def predcit_ampli_along_wav(self, frame_time, wav_name, transform):
...
def process_fun(chunk_queue, process_num, hash_str, return_dict):
    ...
    with tf.device(dev):
        ...
        while not chunk_queue.empty():
            ...
            label_str_list = []
            bad_count_list = []
            sp_res = spk_sp(chunk_name_list, cursor, cfg_list)
            print sp_res
            for chunk_name in chunk_name_list:
                chunk_name_c = chunk_name[0]
                pa_r = pa.predcit_ampli_along_wav(1.6, chunk_name_c, transform)
                if pa_r == -1:
