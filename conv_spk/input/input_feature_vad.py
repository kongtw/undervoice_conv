...
if __name__ == '__main__':
    ...
    dataset = AudioDataset(files_path=args.file_path, audio_dir=args.audio_dir,
                           transform=Compose(
                               [CMVN(), Feature_longest(cube_shape=(20, 80, 40), cube_num=100, augmentation=True),
                                ToOutput()]))
    ...
    idx = 0
    num_subjects = 511
    # feature = np.transpose(np.array([dataset.__getitem__(idx)[0][0] for idx in range(num_subjects)]), (0, 2, 3, 1))
    feature = np.transpose(np.array([dataset.__getitem__(idx)[0][0] for idx in range(num_subjects)]), (0, 1, 3, 4, 2))
    print(feature.shape)

    feature = np.reshape(feature,
                         [feature.shape[0] * feature.shape[1], feature.shape[2], feature.shape[3], feature.shape[4]])

    #tranpose后,feature的shape是batch*80 * 40 * 20

    print(feature.shape)
    feature_mean = np.mean(feature, axis=0)
    feature_std = np.std(feature, axis=0)

    #mean与std的shape是batch*80 * 40 * 20

    feature -= feature_mean
    feature /= feature_std
    index = random.sample(range(feature.shape[0]), feature.shape[0])
    feature = feature[index]
    label = np.array([dataset.__getitem__(idx)[1] for idx in range(num_subjects)])
    label_test = label
    print(label_test.shape)
    print(label.shape)
    label = np.reshape(label, [label.shape[0], 1])
    label = np.repeat(label, 100, axis=1)
    label = np.reshape(label, [label.shape[0] * label.shape[1]])
    print(label.shape)
    label = label[index]

    feature_test = np.transpose(np.array([dataset_test.__getitem__(idx)[0][0] for idx in range(num_subjects)]),
                                (0, 2, 3, 1))
    print(feature_test.shape)
    feature_test_mean = np.mean(feature_test, axis=0)
    feature_test_std = np.std(feature_test, axis=0)
    feature_test -= feature_test_mean
    feature_test /= feature_test_std

    with h5py.File('../data/dev_sample_dataset_zs_traintest.hdf5', 'w') as hdf5_f:
        hdf5_f['utterance_train'] = feature
        hdf5_f['label_train'] = label
        hdf5_f['utterance_test'] = feature_test
        hdf5_f['label_test'] = label_test

    feature_mean = np.transpose(feature_mean, axes=(2, 0, 1))
    feature_std = np.transpose(feature_std, axes=(2, 0, 1))
    feature_test_mean = np.transpose(feature_test_mean, axes=(2, 0, 1))
    feature_test_std = np.transpose(feature_test_std, axes=(2, 0, 1))
    np.save('../data/feature_mean.npy', feature_mean)
    np.save('../data/feature_std.npy', feature_std)
    np.save('../data/feature_test_mean.npy', feature_test_mean)
    np.save('../data/feature_test_std.npy', feature_test_std)