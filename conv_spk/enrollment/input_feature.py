...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path',
                        default=os.path.expanduser(
                            '~/python/undervoice_conv/input/file_path.txt'),
                        help='The file names for development phase')

    # The directory of the audio files separated by subject
    parser.add_argument('--audio_dir',
                        default=os.path.expanduser(
                            '~/python/undervoice_conv/input/Audio'),
                        help='Location of sound files')
    args = parser.parse_args()

    dataset_enroll = AudioDataset(files_path=args.file_path, audio_dir=args.audio_dir,
                                  transform=Compose(
                                      [CMVN(), Feature_Cube(cube_shape=(20, 80, 40), augmentation=True), ToOutput()]))
    dataset_eval = AudioDataset(files_path=args.file_path, audio_dir=args.audio_dir,
                                transform=Compose(
    ...
    enroll_begin = 511
    enroll_subjects = 100
    feature_enroll = np.transpose(np.array(
        [dataset_enroll.__getitem__(idx)[0] for idx in range(enroll_begin, enroll_subjects + enroll_begin)]),
        [0, 2, 3, 4, 1])
    print(feature_enroll.shape)
    feature_enroll = np.reshape(feature_enroll,
                                [feature_enroll.shape[0] * feature_enroll.shape[1], feature_enroll.shape[2],
                                 feature_enroll.shape[3], feature_enroll.shape[4]])
    print(feature_enroll.shape)

    feature_eval = np.transpose(np.array(
        [dataset_eval.__getitem__(idx)[0] for idx in range(enroll_begin, enroll_subjects + enroll_begin)]),
        [0, 2, 3, 4, 1])
    print(feature_eval.shape)
    feature_eval = np.squeeze(feature_eval, axis=1)
    print(feature_eval.shape)

    label = np.array([dataset_eval.__getitem__(idx)[1] for idx in range(enroll_begin, enroll_subjects + enroll_begin)])
    label_eval = label
    print(label_eval.shape)
    label = np.reshape(label, [label.shape[0], 1])
    print(label.shape)
    label = np.repeat(label, 20, axis=1)
    label_enroll = np.reshape(label, [label.shape[0] * label.shape[1]])
    print(label_enroll.shape)
    print(label_enroll)

    with h5py.File('../../data/enroll_eval_sample_dataset.hdf5', 'w') as hdf5_f:
        hdf5_f['utterance_enrollment'] = feature_enroll
        hdf5_f['label_enrollment'] = label_enroll
        hdf5_f['utterance_evaluation'] = feature_eval
        hdf5_f['label_evaluation'] = label_eval
