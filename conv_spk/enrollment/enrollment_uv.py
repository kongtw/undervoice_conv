...
tf.app.flags.DEFINE_integer('num_clones_er', 1,
                            'Number of model clones to deploy.')

tf.app.flags.DEFINE_boolean('clone_on_cpu_er', False,
                            'Use CPUs to deploy clones.')
tf.app.flags.DEFINE_boolean('online_pair_selection_er', False,
                            'Use online pair selection.')

tf.app.flags.DEFINE_integer('worker_replicas_er', 1, 'Number of worker replicas.')

tf.app.flags.DEFINE_integer(
    'num_ps_task_ers_er', 0,
...
tf.app.flags.DEFINE_integer(
    'num_preprocessing_threads_er', 8,
    'The number of threads used to create the batches.')

tf.app.flags.DEFINE_integer(
    'log_every_n_steps_er', 20,
    'The frequency with which logs are print.')

tf.app.flags.DEFINE_integer(
    'save_summaries_secs_er', 10,
    'The frequency with which summaries are saved, in seconds.')

tf.app.flags.DEFINE_integer(
    'save_interval_secs_er', 500,
    'The frequency with which the model is saved, in seconds.')

tf.app.flags.DEFINE_integer(
    'task_er', 0, 'task id of the replica running the training.')
...
tf.app.flags.DEFINE_string(
    'learning_rate_decay_type_er',
    'exponential',
    'Specifies how the learning rate is decayed. One of "fixed", "exponential",'
    ' or "polynomial"')

tf.app.flags.DEFINE_float('learning_rate_er', 1.0, 'Initial learning rate.')

tf.app.flags.DEFINE_float(
    'end_learning_rate_er', 0.0001,
    'The minimal end learning rate used by a polynomial decay learning rate.')

tf.app.flags.DEFINE_float(
    'label_smoothing_er', 0.0, 'The amount of label smoothing.')

tf.app.flags.DEFINE_float(
    'learning_rate_decay_factor_er', 0.94, 'Learning rate decay factor.')

tf.app.flags.DEFINE_float(
    'num_epochs_per_decay_er', 2.0,
    'Number of epochs after which learning rate decays.')

tf.app.flags.DEFINE_bool(
    'sync_replicas_er', False,
    'Whether or not to synchronize the replicas during training.')

tf.app.flags.DEFINE_integer(
    'replicas_to_aggregate_er', 1,
    'The Number of gradients to collect before updating params.')

tf.app.flags.DEFINE_float(
    'moving_average_decay_er', None,
    'The decay to use for the moving average.'
    'If left as None, then moving averages are not used.')

tf.app.flags.DEFINE_string(
    'model_speech_er', 'cnn_speech', 'The name of the architecture to train.')

tf.app.flags.DEFINE_integer(
    'batch_size_er', 1, 'The number of samples in each batch.')

tf.app.flags.DEFINE_integer(
    'num_epochs_er', 50, 'The number of epochs for training.')

tf.app.flags.DEFINE_integer('num_gpus_er', 1,
...
class spk_er_conv:
    def __init__(self, process_num, class_num, is_training):
        ...
        with self.graph.as_default():
            ...
            model_speech_er_fn = nets_factory_spk.get_network_fn(
                FLAGS.model_speech_er,
            ...
            self.batch_speech, self.batch_labels = tf.train.batch(
                ...
                num_threads=FLAGS.num_preprocessing_threads_er,
                capacity=5 * FLAGS.batch_size_er)
            with tf.variable_scope(tf.get_variable_scope()):
                for i in xrange(FLAGS.num_clones_er):
                    with tf.device('/gpu:%d' % i):
                        with tf.name_scope('%s_%d' % ('tower', i)) as scope:
                            features, logits, end_points_speech = model_speech_er_fn(self.batch_speech)
    ...
    def er_kf(self, speech, is_training):
        features = self.sess.run(
            self.features,
            feed_dict={self.is_training: is_training, self.batch_dynamic: FLAGS.batch_size_er, self.batch_speech: speech})
        return features

    def er_wav(self, min_time, wav_name, transform, feature_mean, feature_std):
        ...
        if len(logenergy_lmfe) < min_frame_num:
            ...
            feature_array = self.er_kf(feature, False)
            return [1, feature_array]


def enroll_kf(er_wav_path):
    if FLAGS.num_gpus_er == 0:
        dev = '/cpu:0'
    elif FLAGS.num_gpus_er == 1:
        dev = '/gpu:0'
    else:
        raise ValueError('Only support 0 or 1 gpu.')

    er_conv = spk_er_conv(8, 511, False)
    ...
    with tf.device(dev):
        for kf_wav in glob.glob('../input/Audio/subject_vad/*.wav'):
            feature_array = er_conv.er_wav(0.8, kf_wav, transform, feature_mean, feature_std)
            print(feature_array)
            np.save(FLAGS.enrollment_dir_spk + kf_wav.split('/')[-1].split('.')[0] + '.npy', feature_array)
