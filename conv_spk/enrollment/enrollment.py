...
tf.app.flags.DEFINE_string(
    ...
    # 'Directory where checkpoints and event logs are written to.')
    'development_dataset_path', '../data/dev_sample_dataset.hdf5',
    'Directory where checkpoints and event logs are written to.')

tf.app.flags.DEFINE_string(
    # 'enrollment_dataset_path', '../../data/enrollment-evaluation_sample_dataset.hdf5',
    # 'Directory where checkpoints and event logs are written to.')
    'enrollment_dataset_path', '../data/enroll_eval_sample_dataset.hdf5',
    'Directory where checkpoints and event logs are written to.')
...
def main(_):
            ...
            for i in xrange(FLAGS.num_clones):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('%s_%d' % ('tower', i)) as scope:
                        ...
                        label_onehot = tf.one_hot(tf.squeeze(batch_labels, [1]), depth=num_subjects_development,
                                                  axis=-1)
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for speaker_id, speaker_class in enumerate(range(511, 611)):
            ...
            assert len(
                speaker_index) >= NumUtterance, "At least %d utterances is needed for each speaker" % NumUtterance
            ...
            speaker_enrollment, label_enrollment = fileh.root.utterance_enrollment[start_idx:end_idx + 1, :, :,
                                                   0:1], fileh.root.label_enrollment[start_idx:end_idx + 1]
        ...
        np.save(os.path.join(FLAGS.enrollment_dir, 'MODEL_uv.npy'), MODEL)
