...
tf.app.flags.DEFINE_integer(
    'batch_size', 100,
    'The number of samples in each batch. It will be the number of samples distributed for all clones.')

tf.app.flags.DEFINE_integer(
    'num_epochs', 1000, 'The number of epochs for training.')
            ...
            for i in range(FLAGS.num_clones):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('%s_%d' % ('tower', i)) as scope:
                        ...
                        ###################################
                        # one_hot labeling
                        label_onehot = tf.one_hot(tf.squeeze(batch_labels[i * step: (i + 1) * step], [1]),
                                                  depth=num_subjects, axis=-1)
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for i in range(num_batches_per_epoch_test):
                ...
                # Evaluation
                logit, loss_value, test_accuracy, _ = sess.run([logits, loss, accuracy, is_training],
            ...
            for i in range(K):
                Accuracy[i, :] = 100 * np.mean(
                    test_accuracy_vector[i * batch_k_validation:(i + 1) * batch_k_validation], axis=0)
