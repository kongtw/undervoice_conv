...
cursor.execute('SELECT specgram_name FROM features')
...
image_count = 0
newfileflag = False
# f = open('./inputdata/spectro-5-batches-bin-color/data_batch_1.bin', 'ab')
...
for row in rows_random:
    ...
    if newfileflag:
        f.close()
        batch_file_name = './inputdata/spectro-5-batches-bin-color/data_batch_%d.bin' % batch_file_counter
